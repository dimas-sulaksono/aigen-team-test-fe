...
import { updatePaymentType } from "@/services/paymentType";
import { updateSchoolYear } from "@/services/schoolYear";
import React from "react";
import { IoMdClose } from "react-icons/io";

const EditPaymentTypeModal = ({ onClose, data, dispatch, onRefresh }) => {
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updatePaymentType(data.id, name);
      if (response.status) {
        onClose();
        onRefresh();
        dispatch(
          ...
          <div className="mb-4 grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <Input
                ...
                defaultValue={data.paymentTypeName}
                required
              />
            </div>
          </div>
