...
import { deletePaymentType } from "@/services/paymentType";
import { deleteSchoolYear } from "@/services/schoolYear";
import React from "react";
import { MdClose } from "react-icons/md";
const DeletePaymentTypeModal = ({ data, onClose, dispatch, onRefresh }) => {
  const handleDelete = async () => {
    try {
      const response = await deletePaymentType(data.id);
      if (response.status) {
        dispatch(
      ...
      <div className="relative w-full max-w-md rounded-lg bg-white p-4 shadow-md">
        ...
        <div className="p-4 text-center">
          ...
          <h3 className="mb-5 text-lg font-normal text-gray-500">
            Are you sure you want to delete <b>{data.paymentTypeName}</b>?
          </h3>
